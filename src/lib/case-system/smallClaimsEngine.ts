...
export function runSmallClaimsEngine(input: SmallClaimsEngineInput): AnalysisResult {
  ...
  const risksAndGaps = cleanList([
    ...
    // A `profile.judgeConcerns` spread sat here, prefixing each entry with
    // "Judge concern: " and putting it into risksAndGaps — an AnalysisResult
    // field, so this reached the UI. The entries were authored predictions of
    // what a judge would ask ("Is the amount reasonable and supported by
    // documents?"). Nothing replaces it: the factual content was already a
    // duplicate of `coreProofFocus`, which `guidance` below still renders as
    // "Main proof focus", and of the plaintiff elements in claimTypes.ts.
