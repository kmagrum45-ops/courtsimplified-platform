...
### Statement of Claim drafting (not form-filling) — planned, not started
**Correction to how this item was previously understood:** no entry describing this feature existed anywhere in this file (or elsewhere in `docs/`) before this session — searched, not found. It had apparently been talked about as "generate/fill Form 7A," which is the wrong mental model and is being corrected here at first filing rather than fixed in an existing entry.
**What it actually is:** a drafting feature, not a form-filler. It takes the user's own story and specifics already captured in intake — dates, amounts, what happened, in what order — and writes them into the prose structure a Statement of Claim requires: numbered particulars, formal court-document language. The user then reviews, edits, and approves the draft before it's used. This is a different mechanism from `/api/generate-form` (the real PDF-filling engine audited earlier this session — AcroForm field-fill or coordinate-overlay onto an official PDF, no prose generation) and should not be described in those terms.
**The boundary, checked against CLAUDE.md's "who does the applying" test:** the system organizes and formalizes facts the user already stated into correct court-document structure and language — that's legal information/formatting, not legal advice. It does not supply legal characterization the user didn't provide. Where a legal theory or claim-type label is needed (e.g. "breach of contract"), it comes from the existing deterministic claim-type matching already in the codebase (`claimTypeMatcher.ts` / `claimTypes.ts`), not from the drafting step inventing or arguing one itself. The review-and-approve step is CLAUDE.md section 4's "suggest, never decide" principle, already applied elsewhere to single fields, extended here to a full document.
**Existing code to read first, not assessed here:** `src/lib/case-system/claimDraftEngine.ts` already exists and is structurally closer to this drafting feature than a form-filler — it was flagged as live during this session's read-only forms-pipeline audit. Commit `d1fa87c` (the section-3 cleanup session) removed its "LIKELY DEFENCE RISKS"/"LIKELY JUDGE CONCERNS" sections (confirmed: 19 lines removed from this file in that commit), leaving the file's core drafting structure intact but the prohibited content stripped. Whichever future session builds this should read that file first rather than starting from scratch; its current state (what's left, what still needs building) is that session's first step, not assessed as part of this filing.
**Broader context, filed here but out of scope for this item:** the site owner described this drafting piece as one component of a larger intended capability — arranging evidence, statements, witnesses, and other court-package components together, not just one document. That's a separate, larger design conversation for later. This item stays scoped to the Statement of Claim drafting piece only.
**Status:** open, planned, not started. No design doc, no schema, no code written or changed this session — filing only.
